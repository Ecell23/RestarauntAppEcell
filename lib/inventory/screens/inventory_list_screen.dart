      ...
      body: Column(
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                // The new, smaller veg toggle
                FilterChip(
                  label: const Text('Veg Only'),
                  avatar: Icon(Icons.eco, color: _showVegOnly ? Colors.white : Colors.green),
                  selected: _showVegOnly,
                  onSelected: (selected) => setState(() => _showVegOnly = selected),
                  selectedColor: Colors.green,
                  checkmarkColor: Colors.white,
                  labelStyle: TextStyle(color: _showVegOnly ? Colors.white : Colors.black),
                ),
                const SizedBox(width: 8),
                // The category list now takes up the remaining space
                Expanded(
                  child: SizedBox(
                    height: 40,
                    child: ListView.builder(
                      scrollDirection: Axis.horizontal,
                      itemCount: ['All', ..._allCategories..sort()].length,
                      itemBuilder: (context, index) {
                        final category = ['All', ..._allCategories..sort()][index];
                        return Padding(
